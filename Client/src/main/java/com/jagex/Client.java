        ...
        try {
            ...
            if (cache.getIndexedFileSystem().is317()) {
                ...
                try {
                    ...
                    mapFunctions = lastIdx == -1 ? new Sprite[0] : Arrays.copyOf(sprites, lastIdx + 1);
                } catch (Exception ex) {
            ...
            ClientPluginLoader.pluginSet.forEach(plugin -> {
                try {
	...
	*/
    ...
    public final void processLoadedResources(ResourceResponse response) {
        ...
        try {
            ...
            ClientPluginLoader.pluginSet.forEach(plugin ->
                    plugin.onResourceDelivered(response));
