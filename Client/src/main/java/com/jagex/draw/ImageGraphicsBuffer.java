...
public class ImageGraphicsBuffer extends ProducingGraphicsBuffer {
    ...
    public void finalize() {
        IntStream.range(0, pixels.length).parallel().forEachOrdered(index -> {
            pixels[index] = 0xFF000000 | pixels[index];
        });

        finalImage.getPixelWriter().setPixels(0, 0, getWidth(), getHeight(),
                PixelFormat.getIntArgbInstance(), IntBuffer.wrap(pixels), getWidth());
    }
